    ...
    <head>
        ...
        <script>
            function doDelete(id) {
                ...
                if (c) {
                    window.location.href = "blogDelete?id=" + id;
                }
            }
            function doEdit(id) {
                window.location.href = "blogUpdate?id=" + id;
            }
            function doInsert() {
                window.location.href = "insertBlog";
            }
        </script>
    </head>
    <body>
...
<c:forEach items="${requestScope.blogs}" var="b">
    <tr>
        <td><a href="blogDetail?id=${b.id}">${b.title}</a></td>
        <td>${b.from}</td>
        <td><input type="button" value="Edit" onclick="doEdit(${b.id})"></td>
        <td><input type="button" value="Delete" onclick="doDelete(${b.id})"></td>
    </tr>
</c:forEach>
</table>
<br><input type="button" value="New Blog" onclick="doInsert()">
</body>
</html>-->
            ...
            <section class="cta-section theme-bg-light py-5">
                <div class="container text-center single-col-max-width">
                    ...
                    <div class="single-form-max-width pt-3 mx-auto">
                        <form class="signup-form row g-2 g-lg-2 align-items-center" action="search">
                ...
                <c:if test="${requestScope.totalpage > 1}">
                    <div class="container single-col-max-width">
                        <nav class="blog-nav nav nav-justified my-5">
                            <a class="nav-link-prev nav-item nav-link rounded-left" onclick="doPrev(${requestScope.pageindex})">Previous<i class="arrow-prev fas fa-long-arrow-alt-left"></i></a>
                            <span class="nav-link-prev nav-item nav-link rounded">${requestScope.pageindex}</span>
                            <a class="nav-link-next nav-item nav-link rounded-right" onclick="doNext(${requestScope.pageindex}, ${requestScope.totalpage})">Next<i class="arrow-next fas fa-long-arrow-alt-right"></i></a>
                        </nav>
                    </div>
                </c:if>
            </section>
